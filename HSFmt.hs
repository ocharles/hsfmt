  ...
  pp HsModule{..} =
   (case hsmodName of
     Just name ->
       ...
       line <>
       line) <>
   (vsep (mapM pp hsmodImports)) <> line <> line <>
...
instance Print name => Print (TyClDecl name) where
  ...
  pp DataDecl{..} =
    ...
    in (case dd_ND of
       ...
       pp tcdLName <+>
       pp tcdTyVars <+>
       equals <+> pp (ConstructorList dd_cons)

instance Print name => Print (LHsQTyVars name) where
  pp HsQTvs{..} =
    hsep (mapM pp hsq_explicit)
...
instance Print thing => Print (ConstructorList thing) where
  pp (ConstructorList as) =
    cat (punctuate (string "|")
                   (mapM pp as))
...
instance (Print idL,Print idR) => Print (HsBindLR idL idR) where
  pp FunBind{..} =
    vsep (mapM (pp . SingleMatch fun_id)
               (unLoc (mg_alts fun_matches)))
  pp PatBind{..} =
    pp pat_lhs <+> pp pat_rhs
  pp VarBind{..} = error "VarBind"

instance Print name => Print (ConDeclField name) where
  pp ConDeclField{..} = pp (CommaList cd_fld_names) <+> string "::" <+> pp cd_fld_type

instance (Print id, Print body) => Print (MatchGroup id body) where
  pp MG{..} = vsep (mapM pp (unLoc mg_alts))

-- | GHC groups all alternatives for a function under a single match group, but
-- syntactically, we need to mention the function name for every alternative.
data SingleMatch name id body =
  SingleMatch {singleMatchName :: name
              ,singleMatchAlt :: LMatch id body}

instance (Print name, Print id, Print body) => Print (SingleMatch name id body) where
...
instance (Print body, Print id) => Print (GRHS id body) where
  ...
  pp (GRHS guards body) =
    do PrintState{bindSymbol} <- ask
       hang 2 (string "|" <+> pp (CommaList guards) <+> bindSymbol <+> indent 2 (pp body))
...
instance Print name => Print (CommaList name) where
  pp (CommaList names) =
    hang 0 $
    cat (punctuate (comma <> space)
                   (mapM pp names))

instance Print name => Print (Sig name) where
  pp (TypeSig names sig) =
    hang 2 $ pp (CommaList names) <$> string "::" <+> group (pp sig)
  pp (ClassOpSig isDefault names sig) =
    hang 2 $
    (if isDefault
        then string "default" <> space
        else empty) <>
    pp (CommaList names) <$>
    string "::" <+> group (pp sig)
...
instance Print name => Print (HsExpr name) where
  ...
  pp (HsIf _ cond a b) =
    string "if" <+>
    pp cond <+> string "then" <+> pp a <+> string "else" <+> pp b
  pp (HsLet binds expr) =
    align $
    string "let" <+> align (pp binds) <$>
    string "in" <+> align (pp expr)
  pp (ExplicitList _ _ elems) = lbracket <> pp (CommaList elems) <> rbracket
  pp RecordCon{..} = pp rcon_con_name <+> lbrace <> pp rcon_flds <> rbrace
  pp RecordUpd{..} = pp rupd_expr <+> lbrace <+> pp (CommaList rupd_flds) <+> rbrace
...
instance Print name => Print (Pat name) where
  ...
  pp (ConPatIn name (RecCon hsRecFields@(HsRecFields [] (Just 0)))) = pp name <+> lbrace <> pp hsRecFields <> rbrace
  pp (ConPatIn name (RecCon hsRecFields)) = pp name <> lbrace <> pp hsRecFields <> rbrace
...
instance Print name => Print (HsType name) where
  ...
  pp HsQualTy{..} =
    (case unLoc hst_ctxt of
       [] -> empty
       ctx -> lparen <> pp (CommaList ctx) <> rparen <+> string "=>" <> space) <>
    pp hst_body
...
-- | OccNames that need to be parenthesized if they are operators.
...
instance (Print name, IsSymOcc name) => Print (InfixOccName name) where
  pp (InfixOccName occName)
    | isSymOcc occName = lparen <> pp occName <> rparen
