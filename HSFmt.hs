...
main :: IO ()
main =
  prettyPrintFile "HSFmt.hs" >>= T.putStrLn . displayT . renderPretty 1 80
...
instance (Print thing) => Print (ConstructorList thing) where
  pp (ConstructorList as) =
    cat (punctuate (string "|") (mapM pp as))
...
instance ( Print name
  ...
  pp SingleMatch {..} =
    pp (InfixOccName singleMatchName) <+> pp singleMatchAlt
...
instance (Print name, IsSymOcc name) => Print (Sig name) where
  pp (TypeSig names sig) =
    hang 2 (pp (SimpleTypeSig names sig))
...
instance (Print name, IsSymOcc name) => Print (HsExpr name) where
  ...
  pp (HsLit lit) =
    pp lit
  pp (HsLam mg) =
    string "\\" <> bindRArrow (pp mg)
  pp HsLamCase{} =
    error "HsLamCase"
  pp (HsApp l r) =
    group $ hang 2 (pp l <$> group (pp r))
  pp HsAppType{} =
    error "HsAppType"
  pp HsAppTypeOut{} =
    error "HsAppTypeOut"
  ...
  pp (OpApp a other _ b) =
    error "OpApp with a non-HsVar operator"
  pp (NegApp neg _) =
    string "-" <> pp neg
  pp (HsPar expr) =
    lparen <> pp expr <> rparen
  pp (SectionL a b) =
    lparen <> pp a <+> pp b <> rparen
  pp (SectionR a b) =
    lparen <> pp a <+> pp b <> rparen
  pp (ExplicitTuple exprs _) =
    tupled exprs
  ...
  pp (HsLet binds expr) =
    align $ string "let" <+> bindEquals (align (pp binds)) <$> string
                                                                 "in" <+> align
                                                                            (pp
                                                                               expr)
  pp (ExplicitList _ _ elems) =
    lbracket <> pp (CommaList elems) <> rbracket
  pp RecordCon {..} =
    pp rcon_con_name <+> lbrace <> pp rcon_flds <> rbrace
  pp RecordUpd {..} =
...
instance (Print id, Print arg) => Print (HsRecFields id arg) where
  pp (HsRecFields [] Nothing) =
    empty
  pp (HsRecFields [] (Just 0)) =
    string ".."
  pp (HsRecFields flds Nothing) =
    pp (CommaList flds)
...
instance ( Print idL
  ...
  pp (ValBindsIn binds _) =
    vsep (mapM pp (toList binds))
...
instance Print FastString where
  pp fs =
    string (pack (show (unpackFS fs)))

instance (Print body, Print idL) => Print (StmtLR idL idR body) where
  pp (BodyStmt body _syntax1 _syntax2 _placeholder) =
    pp body
...
instance (Print name) => Print (Pat name) where
  ...
  pp BangPat{} =
    error "BangPat"
  pp (ListPat pats _ _) =
    lbracket <> pp (CommaList pats) <> rbracket
  pp (TuplePat elems _ _) =
    singleLineTuple elems
  pp PArrPat{} =
    error "ParrPat"
  pp (ConPatIn name (PrefixCon args)) =
    pp name <+> hsep (mapM pp args)
  pp (ConPatIn name (InfixCon l r)) =
    pp l <+> pp name <+> pp r
...
instance (Print name, IsSymOcc name) => Print (HsType name) where
       ...
       ctx ->
         (case ctx of
            [one] ->
              pp one
            ts ->
              tupled ts) <+> string "=>" <> space) <> pp hst_body
  pp (HsTyVar name) =
    pp name
  pp (HsAppsTy types) =
    hsep (mapM pp types)
  ...
  pp (HsTupleTy _ ts) =
    tupled ts
  pp (HsParTy name) =
    lparen <> pp name <> rparen
  pp (HsListTy name) =
    lbracket <> pp name <> rbracket

instance (Print name, IsSymOcc name) => Print (HsAppType name) where
  pp (HsAppPrefix (L _ a)) =
    pp a
...
instance Print GHC.OccName where
  pp n =
    string (pack (GHC.occNameString n))
