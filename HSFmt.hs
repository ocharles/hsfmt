...
groupDecls [] = []
groupDecls (x@(L _ (SigD (TypeSig names _))) : xs) = let (binds, rest) = splitNames
                                                                           (fmap
                                                                              unLoc
                                                                              names)
                                                                           xs
                                                     in (x : binds) : groupDecls
                                                                        rest
groupDecls (x : xs) = [x] : groupDecls xs

splitNames :: Eq id => [id] -> [LHsDecl id] -> ([LHsDecl id], [LHsDecl id])
splitNames names [] = ([], [])
splitNames names (x : xs) = case unLoc x of
                              ValD (FunBind {..}) -> if unLoc
                                                          fun_id `elem` names then let (binds, rest) = splitNames
                                                                                                         names
                                                                                                         xs
                                                                                   in ( x : binds
                                                                                      , rest
...
instance (IsSymOcc a, Print a, Eq a) => Print (HsModule a) where
  pp HsModule {..} = (case hsmodName of
                        Just name -> hang 2
                                       (string "module" </> pp
                                                              name <> (case hsmodExports of
                                                                         Nothing -> empty
                                                                         Just (L _ []) -> softline <> string
                                                                                                        "()"
                                                                         Just (L _ (e : es)) -> line <> vsep
                                                                                                          (do
                                                                                                          x <-
                                                                                                            string
                                                                                                              "(" <> space <> pp
...
instance (Print name, IsSymOcc name) => Print (ClsInstDecl name) where
  pp ClsInstDecl {..} = string "instance" <+> pp cid_poly_ty <+> string
                                                                   "where" <$> indent
                                                                                 2
                                                                                 (vsep
                                                                                    (mapM
...
instance (Print name, IsSymOcc name) => Print (TyClDecl name) where
  pp ClassDecl {..} = string "class" <+> pp tcdLName <+> pp tcdTyVars <+> string
                                                                            "where" <$> indent
                                                                                          2
                                                                                          (vsep
                                                                                             (mapM
...
instance (Print name, IsSymOcc name) => Print (ConDecl name) where
  pp ConDeclH98 {..} = pp con_name <+> case con_details of
                                         RecCon xs -> align
                                                        (lbrace <+> pp
                                                                      (CommaList
                                                                         (unLoc
...
instance (Print name, IsSymOcc name) => Print (ConDeclField name) where
  pp ConDeclField {..} = pp (CommaList cd_fld_names) <+> string "::" <+> pp
                                                                           cd_fld_type
...
instance (Print id, Print body, IsSymOcc id) => Print (Match id body) where
  pp Match {..} = cat (mapM (\pat -> pp pat <> space) m_pats) <> (case m_type of
                                                                    Nothing -> empty
                                                                    Just a -> pp
                                                                                a) <> nest
                                                                                        2
...
instance (Print name) => Print (CommaList name) where
  pp (CommaList names) = hang 0 $ cat
                                    (punctuate (comma <> space) (mapM pp names))

instance (Print name, IsSymOcc name) => Print (Sig name) where
  pp (TypeSig names sig) = hang 2 (pp (SimpleTypeSig names sig))
  pp (ClassOpSig isDefault names sig) = hang 2 $ (if isDefault then string
                                                                      "default" <> space else empty) <> pp
                                                                                                          (SimpleTypeSig
                                                                                                             names
                                                                                                             sig)
...
instance ( Print name
  ...
  pp (SimpleTypeSig names ty) = group $ pp
                                          (CommaList
                                             (fmap InfixOccName
                                                names)) <$> string
                                                              "::" <+> group
...
bindEquals = withReader (\s -> s { bindSymbol = string "=" })
...
instance (Print name, IsSymOcc name) => Print (HsExpr name) where
  ...
  pp (HsDo _ctx statements _) = string "do" <$> vsep
                                                  (mapM pp (unLoc statements))
  pp (HsCase expr patterns) = hang 2
                                (string "case" <+> pp expr <+> string
                                                                 "of" <$> (bindRArrow
                                                                             (pp
                                                                                patterns)))
  pp (HsIf _ cond a b) = string "if" <+> pp cond <+> string "then" <+> pp
                                                                         a <+> string
                                                                                 "else" <+> pp
                                                                                              b
  pp (HsLet binds expr) = align $ string "let" <+> bindEquals
                                                     (align
                                                        (pp binds)) <$> string
                                                                          "in" <+> align
                                                                                     (pp
  ...
  pp RecordUpd {..} = pp rupd_expr <+> lbrace <+> pp
                                                    (CommaList
                                                       rupd_flds) <+> rbrace
...
instance (Print name) => Print (Pat name) where
  ...
  pp (ConPatIn name (RecCon hsRecFields@(HsRecFields [] (Just 0)))) = pp
                                                                        name <+> lbrace <> pp
                                                                                             hsRecFields <> rbrace
  pp (ConPatIn name (RecCon hsRecFields)) = pp name <> lbrace <> pp
                                                                   hsRecFields <> rbrace
...
instance (Print id, Print arg) => Print (HsRecField' id arg) where
  pp HsRecField {..} = pp
                         hsRecFieldLbl <> (if hsRecPun then empty else space <> equals <+> pp
                                                                                             hsRecFieldArg)
...
instance (Print name, IsSymOcc name) => Print (ImportDecl name) where
  pp ImportDecl {..} = string "import" <+> (if ideclQualified then string
                                                                     "qualified" <> space else empty) <> pp
                                                                                                           ideclName <> (case ideclAs of
                                                                                                                           Just n -> space <> string
                                                                                                                                                "as" <+> pp
                                                                                                                                                           n
                                                                                                                           Nothing -> empty) <> (case ideclHiding of
                                                                                                                                                   Just (hiding, L _ names) -> space <> (if hiding then string
